import os, time, json, threading, uuid, re, smtplib
...
def send_email_alert(subject, body):
    ...
    if not SEND_EMAIL_ALERTS or not SMTP_USER or not SMTP_PASSWORD:
        print("[email] Email alerts disabled or SMTP not configured")
        return False
    ...
    try:
        ...
        <html>
          <body style="font-family: Arial, sans-serif;">
            <h2 style="color: #d32f2f;">🚨 Anomaly Alert - RCA Report</h2>
            <div style="background-color: #f5f5f5; padding: 15px; border-radius: 5px;">
              {body}
            </div>
            <hr>
            <p style="color: #666; font-size: 12px;">Generated by Metrics Observability Pipeline</p>
          </body>
        </html>
        """
...
def collector(stop):
    ...
    while not stop.is_set():
        ...
        time.sleep(10)  # DEMO: Collect every 10 seconds
    ...
    while not stop.is_set():
        ...
        for (metric, instance), docs in data.items():
            ...
            if abs(z) >= Z_THRESHOLD:
                ...
                # Check if we already alerted for this metric recently (within 2 minutes)
                recent_alert = out.find_one({
                    "metric": metric,
                    "instance": instance,
                    "detected_at": {"$gte": end - timedelta(minutes=2)}
                })
                
                if recent_alert:
                    # Calculate time difference (handle timezone-aware datetimes)
                    alert_time = recent_alert['detected_at']
                    if alert_time.tzinfo is None:
                        # Make timezone-aware if needed
                        alert_time = alert_time.replace(tzinfo=timezone.utc)
                    time_diff = int((end - alert_time).total_seconds())
                    print(f"[anomaly] skipped duplicate: {metric} on {instance} (alerted {time_diff}s ago)")
                    continue
        ...
        time.sleep(20)  # DEMO: Check every 20 seconds
...
Return JSON: {{"root_cause": "explanation", "action": "what to do"}}"""
                ...
                email_subject = f"🚨 RCA Alert: {a['severity'].upper()} - {a['metric']}"
                email_body = f"""
                <h3>Anomaly Details</h3>
                <ul>
                  <li><strong>Metric:</strong> {a['metric']}</li>
                  <li><strong>Instance:</strong> {a['instance']}</li>
                  <li><strong>Severity:</strong> <span style="color: red;">{a['severity'].upper()}</span></li>
                  <li><strong>Expected:</strong> {a['expected']}</li>
                  <li><strong>Actual:</strong> {a['actual']}</li>
                  <li><strong>Deviation:</strong> {a['deviation_pct']}%</li>
                </ul>
                
                <h3>Root Cause Analysis</h3>
                <p>{rca_json.get('root_cause', 'N/A')}</p>
                
                <h3>Recommended Action</h3>
                <p><strong>{rca_json.get('action', 'N/A')}</strong></p>
                """
        ...
        time.sleep(15)  # DEMO: Check every 15 seconds
...
def cleanup(stop):
    ...
    while not stop.is_set():
        ...
        time.sleep(60)  # DEMO: Cleanup every minute
